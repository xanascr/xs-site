...
router.post("/:slug/enroll", optionalAuth, async (req, res) => {
  if (!req.user) return res.status(401).json({ ok: false, error: "Faça login para se inscrever" });
...
router.post("/:slug/lessons/:lessonSlug/complete", optionalAuth, async (req, res) => {
  if (!req.user) return res.status(401).json({ ok: false, error: "Faça login" });
  const course = await Course.findOne({ slug: req.params.slug });
  if (!course) return res.status(404).json({ ok: false, error: "Curso não encontrado" });
...
router.get("/:slug/certificate", optionalAuth, async (req, res) => {
  if (!req.user) return res.status(401).json({ ok: false, error: "Faça login" });
  const course = await Course.findOne({ slug: req.params.slug });
  if (!course) return res.status(404).json({ ok: false, error: "Curso não encontrado" });
