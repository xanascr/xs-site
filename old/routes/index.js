...
router.get("/", (req, res) => {res.render("index", { examples, changelog, benchmarks, page: "home" });
});

router.get("/docs", (req, res) => {res.render("docs", { page: "docs" });
});

router.get("/docs/:doc", (req, res) => {res.render("docs", { doc: req.params.doc, page: "docs" });
});

router.get("/examples", (req, res) => {res.render("examples", { examples, page: "examples" });
});

router.get("/benchmark", (req, res) => {res.render("benchmark", { benchmarks, page: "benchmark" });
});

router.get("/changelog", (req, res) => {res.render("changelog", { changelog, page: "changelog" });
});

router.get("/login", (req, res) => {res.render("login", { page: "login" });
});

async function getEnrollment(courseId, req) {const token = req.headers.cookie?.match(/xs_token=([^;]+)/)?.[1];
  if (!token) return null;
  try {const jwt = (await import("jsonwebtoken")).default;
    const payload = jwt.verify(token, process.env.JWT_SECRET);
    return await Enrollment.findOne({userId: payload.id, courseId }).lean();
  } catch {return null; }
}

router.get("/dashboard", async (req, res) => {try {const token = req.headers.cookie?.match(/xs_token=([^;]+)/)?.[1];
    let userData = null;
    if (token) {const jwt = (await import("jsonwebtoken")).default;
...
router.get("/courses/:slug", async (req, res) => {try {const course = await Course.findOne({slug: req.params.slug, published: true }).lean();
    if (!course) return res.status(404).render("404", {});
    const enrollment = await getEnrollment(course._id, req);
    res.render("courses/show", { course, enrollment, page: "courses" });
  } catch {res.status(404).render("404", {});
  }
});

router.get("/courses/:slug/lessons/:lessonSlug", async (req, res) => {try {const course = await Course.findOne({slug: req.params.slug, published: true }).lean();
    if (!course) return res.status(404).render("404", {});
    const lesson = course.lessons.find(l => l.slug === req.params.lessonSlug);
    if (!lesson) return res.status(404).render("404", {});

    const lessonHtml = lesson.bodyMd ? sanitizeHtml(lesson.bodyMd) : (lesson.bodyHtml || lesson.contentHtml || lesson.content || "");
    const enrollment = await getEnrollment(course._id, req);
...
router.get("/courses/:slug/quiz/:moduleIdx", async (req, res) => {try {const course = await Course.findOne({slug: req.params.slug, published: true }).lean();
    if (!course) return res.status(404).render("404", {});
    const moduleIndex = parseInt(req.params.moduleIdx);
    if (isNaN(moduleIndex)) return res.status(404).render("404", {});
    const quiz = await ModuleQuiz.findOne({courseId: course._id, moduleIndex }).lean();
    if (!quiz) return res.status(404).render("404", {});
    res.render("courses/quiz", { course, quiz, moduleIndex, page: "courses" });
  } catch {res.status(404).render("404", {}); }
});

router.get("/courses/:slug/certificate", async (req, res) => {try {const course = await Course.findOne({slug: req.params.slug, published: true }).lean();
    if (!course) return res.status(404).render("404", {});

    let enrollment = null;
    let certificate = null;
    const token = req.headers.cookie?.match(/xs_token=([^;]+)/)?.[1];
    if (token) {try {const jwt = (await import("jsonwebtoken")).default;
        const payload = jwt.verify(token, process.env.JWT_SECRET);
    ...
    res.render("courses/certificate", { course, enrollment, certificate, page: "courses" });
  } catch {res.status(404).render("404", {});
  }
});

router.get("/certificates/validate", (req, res) => {res.render("certificates/validate", { page: "certificates", result: null });
});

router.get("/signup", (req, res) => {res.render("signup", { page: "signup" });
});

router.get("/settings", (req, res) => {res.render("settings", { page: "settings" });
});

router.get("/admin", (req, res) => {res.render("admin", { page: "admin" });
});

router.get("/donate", (req, res) => {res.render("donate", { page: "donate" });
});

router.get("/privacy", (req, res) => {res.render("privacy", { page: "privacy" });
});
...
router.get("/reset-password", (req, res) => {res.render("reset-password", { page: "reset-password", token: req.query.token || "" });
});

router.get("/reset-password/:token", (req, res) => {res.render("reset-password", { page: "reset-password", token: req.params.token });
});

function sanitizeSearch(q) {return (q || "").replace(/[(){}\[\]"'~*?\\-]/g, " ").trim().slice(0, 100);
}

router.get("/packages", async (req, res) => {try {const q = sanitizeSearch(req.query.q);
    const filter = {status: "approved" };
    if (q) filter.$text = {$search: q };
    const packages = await Package.find(filter).sort({downloads: -1 }).limit(50).lean();
    res.render("packages/index", { packages, query: q, page: "packages" });
  } catch {res.render("packages/index", { packages: [], query: "", page: "packages" });
  }
});

router.get("/packages/dashboard", (req, res) => {res.render("packages/dashboard", { page: "packages" });
});

router.get("/packages/:name", async (req, res) => {try {const pkg = await Package.findOne({name: req.params.name, status: "approved" }).lean();
    ...
    res.render("packages/show", { pkg, versions, page: "packages" });
  } catch (e) {console.error("Error rendering package show:", e);
    res.status(404).render("404", {});
  }
});

router.get("/packages/:name/:version", async (req, res) => {try {const pkg = await Package.findOne({name: req.params.name, status: "approved" }).lean();
    if (!pkg) return res.status(404).render("404", {});

    const reqVer = req.params.version;
    const versionData = (pkg.versions || []).find(v => v.version === reqVer);
    if (!versionData) return res.status(404).render("404", {});
